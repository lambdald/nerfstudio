...
def processing_single_scene(args):
    ...
    for i in range(num_images):
        ...
        for p3d_id in images[i + 1].point3D_ids:
            ...
            transformed = np.matmul(
                extrinsic[i + 1], [points3d[p3d_id].xyz[0], points3d[p3d_id].xyz[1], points3d[p3d_id].xyz[2], 1]
            )
    ...
    for i in range(num_images):
        ...
        if not img_path.endswith(".jpg"):
            ...
            shutil.copyfile(
                os.path.join(image_dir, images[i + 1].name), os.path.join(image_converted_dir, "%08d.jpg" % i)
            )
