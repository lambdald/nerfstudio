...
def get_bbox_from_points(points: np.ndarray, ignore_percentile=0, ignore_minmax_nums=0):
    """_summary_
    ...
    """
    d = points.shape[1]
    points = np.array(points).T  # [D, N]

    bbox = np.zeros((2, d), np.float64)
    if points.size == 0:
        return bbox
    for i in range(d):
        axis_data = np.sort(points[i])[ignore_minmax_nums:-ignore_minmax_nums]
        bbox[:, i] = [np.percentile(axis_data, ignore_percentile), np.percentile(axis_data, 100 - ignore_percentile)]
...
class PointCloud:
    ...
    def get_bbox(self, ignore_percentile=0, ignore_minmax_nums=5) -> AxisAlignedBoundingBox:
        bbox = get_bbox_from_points(self.points.cpu().numpy(), ignore_percentile, ignore_minmax_nums=ignore_minmax_nums)
